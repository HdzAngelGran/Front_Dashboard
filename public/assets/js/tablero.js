...
(function() {
    ...
    toDoButton.addEventListener('click', function() {
        ...
        newList.innerHTML = '<span class="txt">Nueva tarea</span><span class="idTask"></span><a class="up" href="#"></a><a class="down" href="#"></a><a class="delete" onclick="delTask(event, this.parentNode)" href="#"></a>';
        ...
        if (lis.length == 0) {
            ...
            finalID++;
            while (finalID.toString().length < 3) {
                finalID = '0' + finalID;
            }
        }
    ...
    if (localStorage.getItem('listToDo') != null) {
        ...
        if (localStorage.getItem('listUrgent') !== null) {
            ...
            elem('myLists').classList.add('fourCol');
            var newBoard = document.createElement("div");
            newBoard.innerHTML = '<h3>Urgente <span></span></h3><ul id="ul-urgent" class="section" ondrop="drop(event, this)" ondragover="allowDrop(event)" ondragenter="dragEnter(event)" ondragleave="dragLeave(event)"></ul>';
            newBoard.setAttribute('id', 'ur-gent');
            elem('myLists').insertBefore(newBoard, elem('do-ne'));
